...
export default function Footer() {
  ...
  const activeGradient = "linear-gradient(45deg, #f09433, #e6683c, #dc2743, #cc2366, #bc1888)"; // Instagram-style gradient
        ...
        {tabs.map((tab, index) => {
          ...
          return (
            <Link
              key={tab.href}
              href={tab.href}
              className="flex flex-col items-center justify-center relative w-full h-full transition-transform hover:scale-110"
            >
              <tab.icon
                className={`w-6 h-6 transition-all duration-300 stroke-[1.2]`}
                style={{
